...
app.put("/modifyCatHydration", (req, res) => {
  ...
  const isHydrationAutoValue = isHydrationAuto ? 1 : 0;
  ...
  connection.query(
    updateQuery,
    [isHydrationAutoValue, catGoalHydration, catId],
    (err, results) => {
      if (err) {
      ...
      const selectQuery = `SELECT actual_hydration FROM cat_hydration_statistics_${spaceId}_${catId} ORDER BY date DESC LIMIT 1`;
      connection.query(selectQuery, (err, results) => {
        ...
        if (results.length > 0) {
          ...
          const updateStatsQuery = `UPDATE cat_hydration_statistics_${spaceId}_${catId} SET goal_hydration = ?, hydration_guage = ? ORDER BY date DESC LIMIT 1`;
          connection.query(
            updateStatsQuery,
