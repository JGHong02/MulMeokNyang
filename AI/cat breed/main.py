...
transform_rotation = torchvision.transforms.RandomApply(
    [torchvision.transforms.RandomRotation(20)], p=0.2
)
...
test_data = torchvision.datasets.ImageFolder(
    TEST_DATA_DIR, transform=transform_valid, is_valid_file=lambda x: x.endswith(".jpg")
)
...
def train(
    ...
    for epoch in range(n_epochs):
        ...
        if valid_f1_score > best_valid_f1_score:
            ...
            torch.save(
                model.state_dict(), os.path.join(model_save_path, "best_checkpoint.pth")
            )
        print(
            f"Epoch {epoch + 1} F1-score: {valid_f1_score}\t| Best F1-score: {best_valid_f1_score}"
        )
        torch.save(
            model.state_dict(),
