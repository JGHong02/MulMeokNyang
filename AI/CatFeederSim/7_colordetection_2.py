...
if __name__ == '__main__':
    for i in range(num_of_cats):
        df = pd.read_csv(dir_path + f"/catId_{i + 1}/predict/labels/Id_{i + 1}.csv", sep=' ', header=0, names=['class', 'x1', 'y1', 'x2', 'y2', 'file_name'])
        ...
        for j in imgs_list:
            if (crop_cat(img_path + f"/{j}")):
                ...
                src_image = cv2.imread(dir_path + "/out.png")
                ...
                for l in colors:
                    ...
                    df.loc[df[df["file_name"] == j].index.tolist(), f"color_{k + 1}"] = values[0]
                    k += 1
                print(df.loc[df["file_name"] == j])
            else:
                k = 0
                for l in range(5):
                    df.loc[df[df["file_name"] == j].index.tolist(), f"color_{k + 1}"] = np.NaN
                    k += 1
                print(df.loc[df["file_name"] == j])
        
        df.to_csv(dir_path + f"/catId_{i + 1}/predict/labels/Id_{i + 1}.csv", sep=' ', header=True, index=False)
